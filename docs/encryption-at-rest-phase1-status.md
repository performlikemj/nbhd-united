...
### 1. Mint the ~35 real keys (backfill) — VENUE DECISION NEEDED
`python manage.py backfill_tenant_deks --dry-run` (expect ~35, zero Azure/DB writes) → real run (idempotent, per-tenant isolated). It calls `create_tenant_kek`/`wrap_dek` under the **provisioner** identity, which only exists **inside Azure**. Two venues:
- **In-container (preferred, no rule-break):** needs a trigger — a one-off QStash task or a small ops endpoint that runs the backfill logic in the running container (which holds prod DB access + the provisioner MI). `az containerapp exec` is against project rules — don't use it.
- **Operator-local:** JIT-grant your az-login the `nbhd-kek-provisioner` role at the vault, run `backfill_tenant_deks` locally against the **prod** `DATABASE_URL` (forces `DefaultAzureCredential` = your login), then revoke. Sensitive (laptop → prod-DB write) but bounded.
