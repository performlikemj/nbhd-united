...
class ApplySubtaskCreateTest(TestCase):
    ...
    def test_flag_on_subtask_and_evidence_use_background_authoring(self):
        ...
        self.assertEqual(
            transition_action.pii_receipts["evidence"]["state"],
            "placeholder",
        )
        self.assertNotIn("pii_receipts", transition_action.before_state)

    def test_near_limit_subtask_truncates_after_authoring_without_partial_token(self):
        tenant = _make_tenant("sub-limit@x.test")
        tenant.layer1_placeholder_writes = True
        tenant.pii_entity_map = {"[PERSON_1]": {"name": "Amy"}}
        tenant.save(update_fields=["layer1_placeholder_writes", "pii_entity_map"])
        parent = Task.objects.create(tenant=tenant, title="Parent")
        ...
        subtask = Task.objects.get(id=action.task_id)
        self.assertEqual(subtask.title, "x" * 250 + " ")
        self.assertLessEqual(len(subtask.title), Task._meta.get_field("title").max_length)
        self.assertNotIn("[PERSON", subtask.title)
