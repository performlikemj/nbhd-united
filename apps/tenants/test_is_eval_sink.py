"""The ``is_eval_sink`` backfill predicate (migration 0129) and the flag's boundary.

``0129_tenant_is_eval_sink`` adds the field and classifies the EXISTING eval
...
_MIGRATION_PATH = Path(settings.BASE_DIR) / "apps" / "tenants" / "migrations" / "0129_tenant_is_eval_sink.py"


def _load_migration():
    spec = importlib.util.spec_from_file_location("tenants_0129_tenant_is_eval_sink", _MIGRATION_PATH)
...
class IsEvalSinkBackfillTest(TestCase):
    """Runs the REAL migration functions from ``0129_tenant_is_eval_sink``."""
