...
EXCEPTION (PR8 friends DB backstop): three cross-tenant friends tables carry
real, deliberate FORCE-RLS tenant policies (defense in depth for the highest
blast-radius tables). Those MUST survive this boot-time sweep, or the backstop
would be wiped on every deploy — so they are exempted here. Everything else
keeps the pre-existing fleet posture (RLS off; the anon Data API has zero grants
on these tables, so isolation is the accessor's job — see
apps/friends/access.py). Keep this list in sync with the friends RLS policy
migration and apps.tenants.test_public_schema_lockdown's allowlist.
"""

from django.core.management.base import BaseCommand
from django.db import connection

# Tables whose RLS must NOT be disabled at boot (PR8 friends backstop).
