...
def raise_existing_budgets(apps, schema_editor):
    ...
    Tenant.objects.filter(monthly_token_budget=500_000).update(monthly_token_budget=2_000_000)


def lower_existing_budgets(apps, schema_editor):
    Tenant = apps.get_model("tenants", "Tenant")
    Tenant.objects.filter(monthly_token_budget=2_000_000).update(monthly_token_budget=500_000)
