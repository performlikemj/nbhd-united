"""Re-run the public-schema RLS lockdown after adding the wormhole watermark.

PR3 of the Neighborhood layer adds one public table (``friend_wormhole_visits``
via ``friends.0003``). Per the recurring topo-shift hazard (``0066``/``0073``/
``0078``/``0080``/``0083``/``0097``/``0098``), a new migration can push a prior
...
Depends on ``friends.0003`` (creates the table) and the latest ``tenants``
...
REVERSE_SQL = (
    "-- Reversing this migration would leave the wormhole-visit table without "
