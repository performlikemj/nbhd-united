...
def _ensure_oc_state_dir_in_template(app) -> bool:
    ...
    state relocation: the ``oc-state`` EmptyDir volume + mount AND the four
    env vars that point stateDir at it while pinning config + workspace to
    the share. Returns True if anything changed.
    """
    ...
    return mount_changed or env_changed
