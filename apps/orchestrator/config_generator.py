...
_MORNING_BRIEFING_PROMPT_TEMPLATE = (
    ...
    "7b. Check for overnight research: search for documents with kind='ideas' and slug "
    "starting with 'research-' (`nbhd_journal_search` for 'research'). If the background "
    "agent did research on a goal overnight, mention it briefly in the focus section: "
    "'I looked into {goal} overnight — details in your research notes.'\n\n"
...
_HEARTBEAT_CHECKIN_PROMPT = (
    ...
    "If something needs attention, first check if it's already in the daily note "
    "(load via `nbhd_daily_note_get`). Do NOT re-log information already captured. "
    "Then send the user exactly ONE message via `nbhd_send_to_user`. "
...
_BACKGROUND_TASKS_PROMPT = (
    ...
    "## Part 2: Proactive Research (one goal per run)\n"
    "8. Load the user's goals (`nbhd_document_get` with kind='goal', slug='goals').\n"
    "9. Pick ONE active goal that would benefit from research. Prioritize:\n"
    "   - Goals the user mentioned recently (check journal context)\n"
    "   - Goals that have no research yet\n"
    "   - Goals that are stalling (no progress noted in recent daily notes)\n"
    "   - Skip goals that already have fresh research (updated in last 3 days)\n"
    "   - If no goals exist or none need research, skip Part 2 entirely.\n"
    "10. Do 2-4 targeted `web_search` calls related to the goal. Be specific:\n"
    "    - Good: 'average rent 2LDK Kobe Tarumi-ku 2026'\n"
    "    - Bad: 'moving to Kobe' (too vague, wastes a search)\n"
    "11. For each search result you want to include, open it with `web_fetch` "
    "and read the actual content. Extract specific facts, numbers, names, dates.\n\n"
    "⚠️ RESEARCH INTEGRITY RULE:\n"
    "Every fact you save MUST come from a `web_search` or `web_fetch` result in THIS session.\n"
    "- Include the source URL for every finding.\n"
    "- If a search returns nothing useful, say so. Do NOT fill gaps with guesses.\n"
    "- If you are unsure whether something is current, note the date of the source.\n"
    "- NEVER write a research finding without a URL to back it up.\n\n"
    "12. Save findings to a research document:\n"
    "    `nbhd_document_append` with kind='ideas', slug='research-{goal-slug}'\n"
    "    Format:\n"
    "    ### {Date} — {Goal}: {Research angle}\n"
    "    - {Finding 1} (source: {URL})\n"
    "    - {Finding 2} (source: {URL})\n"
    "    - Next: {what to research next time}\n\n"
    "The morning briefing will pick this up and mention it naturally.\n\n"
    "**Do NOT message the user. Do NOT call nbhd_send_to_user. This is a silent background run.**\n"
    "**Do NOT write to tomorrow's daily note. Update tasks, memory, ideas, or research documents "
    "instead — the morning briefing will read those and surface anything relevant.**\n"
)
