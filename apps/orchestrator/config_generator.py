...
_MORNING_BRIEFING_PROMPT_TEMPLATE = (
    ...
    "`hourly.temperature_2m`, `hourly.weather_code`, and `hourly.wind_speed_10m` for the next "
    "~24h (today's waking hours in the user's timezone). Mention intraday timing ONLY when one "
    "of these thresholds fires:\n"
    "   - precipitation_probability ≥ 40% for 2+ consecutive hours → flag the window, peak %, "
    "and local hour (e.g. 'rain ~13:00-16:00, peak 70% at 14:00')\n"
    "   - any hour with weather_code ≥ 95 (thunderstorm) → always mention the hour\n"
    "   - temperature_2m swing ≥ 10°F (≈ 5.5°C) between 07:00 and 22:00 local → flag it\n"
    "   - wind_speed_10m crossing 20 mph (≈ 32 km/h) when earlier hours were calm → flag it\n"
