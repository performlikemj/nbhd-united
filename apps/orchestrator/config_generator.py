...
_MORNING_BRIEFING_PROMPT_TEMPLATE = (
    ...
    "Steps:\n"
    "1. **Read yesterday's daily note first** — use `nbhd_daily_note_get` for yesterday's date. "
    "This is your baseline. Note what was already reported (sports scores, news stories, "
    "tasks mentioned). Do NOT repeat stale information. If a game/result was in yesterday's "
    "briefing, only mention it again if there's a NEW result since then.\n"
    "2. Get weather using web_fetch with the pre-built Open-Meteo URL below. "
    ...
    "3. Check their calendar for today's events and upcoming 48hrs\n"
    "4. Check for important unread emails or messages\n"
    "5. Load recent journal context — what happened yesterday, any carry-over tasks?\n"
    "6. Load the user's goals (`nbhd_document_get` with kind='goal', slug='goals') for active goals context.\n"
    "7. Load the user's tasks document (`nbhd_document_get` with kind='tasks', slug='tasks'). "
...
_EVENING_CHECKIN_PROMPT = (
    ...
    "2. Load yesterday's daily note too (`nbhd_daily_note_get` with yesterday's date). "
    "Check what was reported yesterday — avoid repeating the same items or stale info.\n"
    "3. Load today's journal context (`nbhd_journal_context`) to see what the user did today.\n"
    "4. Load the user's goals document (`nbhd_document_get` with kind='goal', slug='goals').\n"
    "5. Load the user's tasks document (`nbhd_document_get` with kind='tasks', slug='tasks'). "
    "Check which tasks are open (`- [ ]`) vs completed (`- [x]`).\n"
    "6. VERIFICATION — before listing any item as 'not done':\n"
    ...
    "7. Review today's conversations for things the user learned — decisions made, "
    ...
    "8. Fill in the 'evening-check-in' section of today's daily note "
    ...
    "9. Send the user exactly ONE message via `nbhd_send_to_user`. Keep it short and casual:\n"
