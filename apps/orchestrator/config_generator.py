...
_CRON_CONTEXT_PREAMBLE = (
    ...
    "and goals (`nbhd_document_get` kind='goal', slug='goals'). Then cross-"
    "reference each `- [ ]` item against `nbhd_task_list({status: 'done'})` "
    "and `nbhd_task_list({status: 'skipped'})` — when a markdown line still "
    "shows `- [ ]` but the typed lifecycle has the same item marked `done` or "
    "`skipped`, the typed state wins and the markdown is stale: do NOT carry "
    "that item as a reminder or follow-up. The typed list may be empty for "
    "tenants who haven't adopted the typed lifecycle yet — in that case the "
