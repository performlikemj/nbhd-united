...
_CRON_CONTEXT_PREAMBLE = (
    ...
    "2. Establish the user's CURRENT state THIS turn from the live typed "
    "store, not from narration. Call `nbhd_current_status` — the authoritative "
    "as-of-now snapshot: open tasks, active goals, and finance payment "
    "obligations (per-cycle paid/partial/unpaid). If that tool isn't available, "
    "fall back to `nbhd_task_list({status: 'open'})` + "
    "`nbhd_goal_list({status: 'active'})`. This snapshot — NOT the daily note, "
    "USER.md, or memory — is the source of truth for what is still open. Any "
    "legacy `- [ ]` line from `nbhd_document_get` (kind='tasks'/'goal') that is "
    "absent from the snapshot is stale: do NOT carry it as a reminder or "
    "follow-up. The snapshot is empty only for tenants who haven't adopted the "
    "typed lifecycle — only then does the markdown document stand in as the "
    "source of truth.\n"
    ...
    "the user.\n"
    "5. **GROUNDING CONTRACT** — this governs BOTH quantitative claims (counts, "
    "amounts, dates, streaks, totals, durations, percentages, "
    '"X days ago") AND status/existence claims (whether a task, payment, '
    "shipment, errand, or follow-up is still open, due, unpaid, done, or "
    "pending). Every such claim MUST come from a tool-call result returned in "
    "THIS turn. Do not infer from USER.md, the daily note, memory, or prior "
    "context. Do not average or extrapolate. If a query returns zero rows, say "
    "so plainly — don't pad with stale figures. Use the per-domain query tool "
    "for any number you state: `nbhd_gravity_query` for finance (debt, "
    "payments, payoff).\n"
    "   **Do NOT resurface closed work.** Never raise, nudge, re-ask, or list "
    "as a priority/follow-up any task or obligation that the current-state "
    "snapshot from step 2 does not report as open/active/unpaid. An item absent "
    "from the snapshot — or that the user has told you is handled — is DONE: "
    "drop it, even if the daily note, USER.md, or memory still mentions it. The "
    "snapshot is state; the daily note and memory are narration. When finance "
    "is paused the snapshot carries no obligations — in that case do not raise "
    "loans, debt, balances, or payments from any source. USER.md is identity "
    "(goals, persona, voice) — not current state.\n"
    "6. **WHEN A TOOL FAILS** (HTTP 5xx, timeout, exception, missing capability) "
    "— call `nbhd_platform_issue_report` ONCE per failure with "
    "`category='tool_error'` (or `'missing_capability'` / `'auth_error'` if more "
    "specific), the failing `tool_name`, a brief `summary` of what broke, and "
    "`severity` matched to impact. That is the operator's audit trail — it MUST "
    "happen before you generalize. In journal writes, describe the failure by "
    "the specific tool that failed ('calendar lookup unavailable', 'task list "
    "returned an error') and keep going with whatever sections still have data. "
    "NEVER write 'backend completely down', 'system is broken', 'everything is "
    "offline', or any equivalent phrasing — if you are still writing journal "
    "sections, your own runtime IS up and the failure is always narrower than "
...
_MORNING_BRIEFING_PROMPT_TEMPLATE = (
    ...
    "Check which tasks are open (`- [ ]`) vs completed (`- [x]`). Cross-reference each `- [ ]` "
    "item against the typed state via `nbhd_task_list({{status: 'done'}})` and `nbhd_task_list("
    "{{status: 'skipped'}})` — if the typed lifecycle marks the same item done or skipped, "
    "the typed state wins and that markdown line is stale: skip it. Only reference items "
    "that are open in BOTH (or where the typed list is silent because this tenant hasn't "
    "adopted the typed lifecycle).\n"
    ...
    "- If a **North Star** section is present in your loaded context, weigh the day "
    "against it — favour priorities that move the user toward it (one line, no lecture)\n\n"
    "### Quick Wins\n"
    ...
    "Then end that SAME message with a journal deep-link marker on its OWN line, "
    "after a blank line. "
    "The marker is exactly this shape (do NOT wrap it in backticks or a code fence):\n"
    "[[journal-link: daily|<DATE>|Morning Report]]\n"
    "Replace <DATE> with the ISO date (YYYY-MM-DD) of the daily note you wrote in "
    "step 10 — today's date in the user's timezone, which is that note's slug. Never "
    "invent or guess the date; it must be the daily note's actual slug. The marker "
    "renders as a tappable 'View in Journal' chip in the iOS app and is automatically "
    "stripped from the message on Telegram/LINE, so include it on every run. "
    ...
    "the exact final line order is REQUIRED: put the journal-link marker first, "
    "then put the quick-replies marker on the next and final line:\n"
    "[[journal-link: daily|<DATE>|Morning Report]]\n"
    "[[quick-replies: How's my week? | Add a note]]\n"
    "The quick-replies marker must be the very last line because the delivery "
    "parser strips it first, then parses the now-final journal-link line. Do not "
    "reverse these two marker lines.\n\n"
...
_EVENING_CHECKIN_PROMPT = (
    ...
    "Then end that SAME message with a journal deep-link marker on its OWN line, "
    "after a blank line. The marker is exactly this shape (do NOT wrap it in "
    "backticks or a code fence):\n"
    "[[journal-link: daily|<DATE>|Evening Check-in]]\n"
    "Replace <DATE> with today's ISO date (YYYY-MM-DD) in the user's timezone — "
    ...
    "markers, the exact final line order is REQUIRED: put the journal-link marker "
    "first, then put the quick-replies marker on the next and final line:\n"
    "[[journal-link: daily|<DATE>|Evening Check-in]]\n"
...
_PERSONAL_QUESTION_PROMPT = (
    ...
    "Learned`, and `## People & Context`. ALSO call "
    "`nbhd_yesterdays_signals` for a cross-pillar snapshot of yesterday's "
    "activity (Fuel / Journal / Lessons), today_so_far (catches late "
    "logging), and a `notable_gaps` array like `journal_dark_3_days`, "
    "`fuel_quiet_5_days`, or `energy_stale_7_days`. Treat notable_gaps "
    "as a HINT for step 3, not a script — and absence of a flag does NOT "
    "mean 'all good', it just means no threshold tripped.\n"
    ...
    "   f. A NOTABLE SIGNAL GAP from step 1's `notable_gaps` — only when "
    "no a-e option fits and only when a natural framing exists. Frame as "
    "curiosity, NEVER as audit; never quote the flag back; never lead "
    "with 'I noticed' or 'you didn't'. Examples:\n"
    "      - `journal_dark_3_days` → 'How are you holding up? Quiet few "
    "days.' (NOT 'You haven't journaled in 3 days.')\n"
    "      - `fuel_quiet_5_days` → 'Body asking for rest, or did the week "
    "just get loud?' (NOT 'You haven't worked out in 5 days.')\n"
    "      - `energy_stale_7_days` → 'How's the tank running?' (NOT 'You "
    "haven't logged energy in a week.')\n"
    "      If you can't find natural framing, drop it — option a-e or "
    "skip the day. The signal is a prompt for YOU, not content to recite "
    "to the user.\n"
...
_WEEK_AHEAD_REVIEW_PROMPT_TEMPLATE = (
    ...
    "9. **Weekly review fallback:** Weekly document slugs are ISO weeks like `2026-W28`: "
    "first call `nbhd_document_get` with kind='weekly' and no slug to list `available_slugs`, "
    "then get an exact returned slug. Check if a weekly review was saved for last week "
...
_HEARTBEAT_CHECKIN_PROMPT = (
    ...
    "ask the user to fill signal gaps — that is the Personal Question "
    "cron's job. If the only 'new' thing this heartbeat would mention is "
    "a signal-driven gap with no other context from 1-4, stay silent.\n\n"
...
_WEEKLY_REFLECTION_PROMPT = (
    ...
    "   Weekly document slugs are ISO weeks like `2026-W28`: first call `nbhd_document_get` "
    "with kind='weekly' and no slug to list `available_slugs`, then get an exact returned slug.\n"
    ...
    "If a **North Star** section is present in your loaded context, reflect on whether the week "
    "moved the user toward it — surface it gently in the recap, not as a scorecard.\n"
...
_BACKGROUND_TASKS_PROMPT = (
    ...
    "   a. FIRST establish current state from the live typed store. Do NOT add or reopen "
    "anything that already exists in ANY status — including items the user has already "
    "completed or skipped. If an existing entry covers the same intent and its state has "
    "genuinely changed, update that entry in place; never add a second copy.\n"
    "   b. Do NOT create new tasks or goals on this run. Silently minting tasks or goals "
    "from journal prose has repeatedly resurrected items the user already finished — net-new "
...
# Model mapping by tier — DeepSeek V4 Pro is the starter chat primary as of
# 2026-08-09. DeepSeek V4 Flash remains in the allowlist
# (`TIER_MODEL_CONFIGS`) as the selectable low-latency alternative; explicit
# user selections still win in `resolve_tenant_models` below.
...
# remains the interactive chat default while the small worker (Flash) handles
# scheduled legwork.
