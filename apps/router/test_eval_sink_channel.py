...
class ResolveUserChannelMatrixTest(TestCase):
    """Exhaustive {is_eval_sink} × {device token} × {telegram} × {line} matrix.

    Eval-sink ON must resolve to ``"eval"`` for EVERY surface combination — the
    ...
    def _combo(self, *, idx: int, eval_sink: bool, device: bool, telegram: bool, line: bool):
        user = User.objects.create_user(username=f"matrix-{idx}", password="x")
        if telegram:
            user.telegram_chat_id = 1000 + idx
        if line:
            user.line_user_id = f"U_matrix_{idx}"
        user.save()
        tenant = Tenant.objects.create(
            user=user,
            status=Tenant.Status.ACTIVE,
            is_synthetic=eval_sink,
            is_eval_sink=eval_sink,
        )
        if device:
            DeviceToken.objects.create(
                tenant=tenant, user=user, token=f"{idx:02d}" * 32, environment="sandbox", bundle_id="x.y.z"
            )
        return user

    def test_eval_sink_always_wins_regardless_of_linked_surfaces(self):
        idx = 0
        for device in (False, True):
            for telegram in (False, True):
                for line in (False, True):
                    user = self._combo(idx=idx, eval_sink=True, device=device, telegram=telegram, line=line)
                    self.assertEqual(
                        resolve_user_channel(user),
                        "eval",
                        f"eval-sink tenant leaked past the sink (device={device}, telegram={telegram}, line={line})",
                    )
                    idx += 1
    ...
    def test_without_the_flag_the_app_first_order_holds(self):
        ...
        idx = 100
        for (device, telegram, line), want in expected.items():
            user = self._combo(idx=idx, eval_sink=False, device=device, telegram=telegram, line=line)
            self.assertEqual(
                resolve_user_channel(user),
                want,
                f"(device={device}, telegram={telegram}, line={line})",
            )
            idx += 1
