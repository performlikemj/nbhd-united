...
class OAuthAuthorizeViewTest(TestCase):
    ...
    def test_authorize_state_binds_user_and_provider(self):
        response = self.client.get("/api/v1/integrations/authorize/google/")
        ...
        self.assertIn("https://www.googleapis.com/auth/gmail.modify", scope_values)
        self.assertIn("https://www.googleapis.com/auth/calendar", scope_values)
        self.assertIn("https://www.googleapis.com/auth/drive.file", scope_values)
...
class OAuthCallbackViewTest(TestCase):
    ...
    def _state(self, provider: str = "google") -> str:
        self.client.force_authenticate(user=self.tenant.user)
    ...
    def test_callback_passes_provider_email_to_connect(
        ...
        self.assertEqual(
            response["Location"],
            "http://localhost:3000/settings/integrations?connected=google",
        )
    ...
    @patch("apps.integrations.views.connect_integration")
    @patch("apps.integrations.views.httpx.post")
    def test_callback_rejects_replayed_state(self, mock_post, _mock_connect, _mock_email, _mock_update_config):
        mock_response = Mock()
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {
            "access_token": "access-token",
