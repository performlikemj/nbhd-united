...
- **`trends` (the 4-week digest).** Volume (`sessions_28d` / `minutes_28d`, plus this-week `sessions_7d` / `minutes_7d`), `by_category` frequency, `recency_days` (days since last session per activity), `recent_prs` (last 28 days only), and `volume_trend` (`up`/`down`/`flat`). PR rows always include `metric` and a server-authored `display`; use that display rather than relabeling the numeric value. Use these for near-term recommendations — "you've run 3× this week but haven't lifted in 9 days", "volume's trending down, maybe a deload week" — rather than re-deriving them from the raw list.
- **`all_time_prs` (lifetime bests).** The user's full PR list, newest-first — the *same* list the app's PR feed shows, NOT windowed to 4 weeks like `trends.recent_prs`. Use it when the user asks "what's my deadlift PR?" or you want to celebrate a lifetime best. Each row is `{exercise, value, metric, display, date}`. `metric: "est_1rm"` means an **estimate derived from a rep set**, not weight the user actually lifted. Never present it as a bare "<value> kg PR". Say "estimated 1RM"; when congratulating, name the actual `weight × reps` source set from `display` and call the 1RM estimated.
...
> **Weekday keys are NAMES.** `schedule_json` is keyed by `"monday"`, `"tuesday"`, `"wednesday"`, `"thursday"`, `"friday"`, `"saturday"`, `"sunday"` (case-insensitive; `"mon"`..`"sun"` also work). Write the day's name — never a number. Numeric indices are legacy-only: three conventions disagree (Python Mon=0, ISO Mon=1, cron Sun=0), so a "3" you meant as Wednesday is stored as Thursday and the user's workout appears on the wrong day. Same rule inside `week_overrides` values (its top-level keys are still 0-indexed *week* offsets). Two keys naming the same day (`"2"` and `"wednesday"`) are rejected with a 400.

> **Start-anchor rule — enforced.** ALWAYS pass the user's tenant-local start anchor as `start_date`. For "today" / "I'm at the gym now", `start_date` is today and `schedule_json` MUST include today's weekday — rotate the split so today is day 1. This is not advice: if `start_date` is today and that weekday is missing from the cadence, the server returns a **400 naming the day to add**. Fix the schedule and retry; don't move `start_date` to dodge it. Never design a cadence that excludes the day the user asked to start training. The create response returns `first_workout_date` and `start_date_note`; describe the first session using `first_workout_date`, never by assuming `start_date` has a session.
...
- **"Fill in my workouts" / "prescribe exercises for the week"** (only when an **active plan already exists** — otherwise this is a plan-creation request, see *Workout Plan Generation*) → call `nbhd_fuel_summary` for plan ids, then `nbhd_fuel_get_plan` for the full plan. Walk every workout row with `has_prescription: false` and call `nbhd_fuel_update_workout` with populated category-appropriate `detail_json`; `has_prescription: null` marks a rest row, so skip it. Re-check the plan and continue until no false rows remain. Cover **every category**, not just strength — cardio gets `{distance_km, pace}`, HIIT gets `{rounds, work_s, rest_s}`, and mobility gets catalog hold-time movements under `{skills}`. For "this week", `nbhd_fuel_audit.next_14d_workouts` carries the same flag. If a write returns `unmatched_exercises`: for movements YOU chose, search the catalog and use the exact catalog name; for movements the USER asked for by name, keep the user's name (no figure is fine) — never swap a user-requested movement without asking.
