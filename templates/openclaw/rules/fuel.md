...
- **`trends` (the 4-week digest).** Volume (`sessions_28d` / `minutes_28d`, plus this-week `sessions_7d` / `minutes_7d`), `by_category` frequency, `recency_days` (days since last session per activity), `recent_prs` (last 28 days only), and `volume_trend` (`up`/`down`/`flat`). Use these for near-term recommendations — "you've run 3× this week but haven't lifted in 9 days", "volume's trending down, maybe a deload week" — rather than re-deriving them from the raw list.
