import type { Config } from "tailwindcss";
import typography from "@tailwindcss/typography";

const config: Config = {
  content: [
    "./app/**/*.{js,ts,jsx,tsx,mdx}",
    "./components/**/*.{js,ts,jsx,tsx,mdx}",
    "./lib/**/*.{js,ts,jsx,tsx,mdx}",
  ],
  theme: {
    extend: {
      spacing: {
        "fluid-xs": "var(--space-xs)",
        "fluid-sm": "var(--space-sm)",
        "fluid-md": "var(--space-md)",
        "fluid-lg": "var(--space-lg)",
        "fluid-xl": "var(--space-xl)",
        "page": "var(--page-pad)",
      },
      colors: {
        "os-sky": "var(--os-sky)",
        "os-ink": "var(--os-ink)",
        "os-muted": "var(--os-muted)",
        "os-faint": "var(--os-faint)",
        "os-label": "var(--os-label)",
        "os-hairline": "var(--os-hairline)",
        "os-ring": "var(--os-ring)",
        "os-surface": "var(--os-surface)",
        "os-surface-solid": "var(--os-surface-solid)",
        "os-accent": "var(--os-accent)",
        "os-accent-line": "var(--os-accent-line)",
        "os-accent-soft": "var(--os-accent-soft)",
        "os-done": "var(--os-done)",
        "os-attn": "var(--os-attn)",
        "os-danger": "var(--os-danger)",
        "os-bar": "var(--os-bar)",
        ink: "var(--ink)",
        "ink-muted": "var(--ink-muted)",
        "ink-faint": "var(--ink-faint)",
        mist: "var(--mist)",
        bg: "var(--bg)",
        surface: "var(--surface)",
        "surface-elevated": "var(--surface-elevated)",
        "surface-hover": "var(--surface-hover)",
        card: "var(--card)",
        accent: "var(--accent)",
        "accent-hi": "var(--accent-hi)",
        "accent-hover": "var(--accent-hover)",
        signal: "var(--signal)",
        "signal-text": "var(--signal-text)",
        "signal-faint": "var(--signal-faint)",
        "engagement-show-up": "var(--engagement-show-up)",
        "engagement-move": "var(--engagement-move)",
        "engagement-sit": "var(--engagement-sit)",
        "engagement-reflect": "var(--engagement-reflect)",
        "engagement-rest": "var(--engagement-rest)",
        telegram: "var(--telegram)",
        line: "var(--line)",
        border: "var(--border)",
        "border-strong": "var(--border-strong)",
        "rose-bg": "var(--rose-bg)",
        "rose-border": "var(--rose-border)",
        "rose-text": "var(--rose-text)",
        "amber-bg": "var(--amber-bg)",
        "amber-border": "var(--amber-border)",
        "amber-text": "var(--amber-text)",
        "emerald-bg": "var(--emerald-bg)",
        "emerald-text": "var(--emerald-text)",
        overlay: "var(--overlay)",

        "status-emerald": "var(--status-emerald-bg)",
        "status-emerald-text": "var(--status-emerald-text)",
        "status-rose": "var(--status-rose-bg)",
        "status-rose-text": "var(--status-rose-text)",
        "status-amber": "var(--status-amber-bg)",
        "status-amber-text": "var(--status-amber-text)",
        "status-sky": "var(--status-sky-bg)",
        "status-sky-text": "var(--status-sky-text)",
        "status-slate": "var(--status-slate-bg)",
        "status-slate-text": "var(--status-slate-text)",
        "status-indigo": "var(--status-indigo-bg)",
        "status-indigo-text": "var(--status-indigo-text)",
        "status-violet": "var(--status-violet-bg)",
        "status-violet-text": "var(--status-violet-text)",
        "status-orange": "var(--status-orange-bg)",
        "status-orange-text": "var(--status-orange-text)",

        "c-purple": "var(--constellation-purple)",
        "c-pink": "var(--constellation-pink)",
        "c-teal": "var(--constellation-teal)",
        "c-dark": "var(--constellation-dark)",
        "c-surface": "var(--constellation-surface)",
        "c-border": "var(--constellation-border)",
        "c-text": "var(--constellation-text)",
        "c-text-muted": "var(--constellation-text-muted)",
        "c-text-faint": "var(--constellation-text-faint)",
      },
      fontFamily: {
        body: ["var(--font-body)", "sans-serif"],
        display: ["var(--font-display)", "serif"],
        mono: ["var(--font-mono)", "monospace"],
        headline: ["var(--font-headline)", "sans-serif"],
        serif: ["var(--font-serif)", "Georgia", "serif"],
      },
      boxShadow: {
        panel: "var(--shadow-panel)",
      },
      gridTemplateColumns: {
        "10": "repeat(10, minmax(0, 1fr))",
        "15": "repeat(15, minmax(0, 1fr))",
        "30": "repeat(30, minmax(0, 1fr))",
      },
      borderRadius: {
        panel: "1.25rem",
      },
      keyframes: {
        reveal: {
          "0%": { opacity: "0", transform: "translateY(14px)" },
          "100%": { opacity: "1", transform: "translateY(0)" },
        },
        pulseGrid: {
          "0%, 100%": { opacity: "0.55" },
          "50%": { opacity: "0.8" },
        },
        twinkle: {
          "0%, 100%": { opacity: "0.2" },
          "50%": { opacity: "0.8" },
        },
        float: {
          "0%, 100%": { transform: "translateY(0)" },
          "50%": { transform: "translateY(-6px)" },
        },
        constellationShimmer: {
          "0%, 100%": { opacity: "0.1" },
          "50%": { opacity: "0.28" },
        },
        rewardBurst: {
          "0%": { opacity: "0", transform: "scale(0.72)" },
          "55%": { opacity: "1", transform: "scale(1.06)" },
          "100%": { opacity: "1", transform: "scale(1)" },
        },
        rewardRay: {
          "0%": { opacity: "0", transform: "scaleY(0.2)" },
          "45%": { opacity: "0.7", transform: "scaleY(1)" },
          "100%": { opacity: "0", transform: "scaleY(0.65)" },
        },
      },
      animation: {
        reveal: "reveal 420ms ease-out both",
        "reveal-1": "reveal 420ms ease-out 100ms both",
        "reveal-2": "reveal 420ms ease-out 200ms both",
        "reveal-3": "reveal 420ms ease-out 350ms both",
        "reveal-4": "reveal 420ms ease-out 500ms both",
        pulseGrid: "pulseGrid 7s ease-in-out infinite",
        twinkle: "twinkle 3s ease-in-out infinite",
        "twinkle-slow": "twinkle 5s ease-in-out infinite",
        "twinkle-fast": "twinkle 2s ease-in-out infinite",
        float: "float 6s ease-in-out infinite",
        "constellation-shimmer": "constellationShimmer 3.6s ease-in-out infinite",
        "reward-burst": "rewardBurst 680ms ease-out both",
        "reward-ray": "rewardRay 900ms ease-out both",
      },
    },
  },
  plugins: [typography],
};

export default config;
